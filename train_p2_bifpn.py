...
# -*- coding: utf-8 -*-
"""
训练脚本 - YOLOv8s with P2 + BiFPN-Lite
在 P2 模型基础上，将 PAN 替换为轻量化的 BiFPN，提升特征融合能力
"""
import os
import sys
...
def main():
    ...
    model_yaml = project_root / "cfg" / "models" / "yolov8s-p2-bifpn.yaml"
    ...
    train_args = {
        ...
        'name': 'y8s_p2_bifpn_1024_adamw',
        'imgsz': 1024,
        'epochs': 200,
        'batch': 4,  # BiFPN 增加了一些计算，保持 batch=4
        'workers': workers,
        'device': 0,
        'amp': True,
        'seed': 42,
    ...
    print("\n开始训练 YOLOv8s-P2-BiFPN...")
    print("=" * 70)
    print(f"项目: {train_args['project']}")
    print(f"实验名: {train_args['name']}")
    print(f"改进: +P2 Detection Head + BiFPN-Lite")
